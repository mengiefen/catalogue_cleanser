...
class DictionaryEntriesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @dictionary_entry.save
        format.html do
          redirect_to(dictionary_entry_url(@dictionary_entry), notice: 'Dictionary entry was successfully created.')
        end
  ...
  def update
    respond_to do |format|
      if @dictionary_entry.update(dictionary_entry_params)
        format.html do
          redirect_to(dictionary_entry_url(@dictionary_entry), notice: 'Dictionary entry was successfully updated.')
        end
  ...
  def dictionary_entry_params
    params.require(:dictionary_entry).permit(:phrase, :canonical)
  end
end
